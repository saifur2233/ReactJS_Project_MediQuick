...
const CheckAuthenticity = () => {
  ...
  const { data: drug = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://mediquick-server.onrender.com/api/v1/customer/${drugCode}`
      );
  ...
  const { data: drugJourneyDetails = [], isloading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://mediquick-server.onrender.com/api/v1/drug-journey/${drugCode}`
      );
      const data = await res.json();
      return data;
    },
  ...
  // useEffect(() => {
  //   fetch(`https://mediquick-server.onrender.com/api/v1/drug-journey/${drugCode}`)
  //     .then((res) => res.json())
  //     .then((data) => {
