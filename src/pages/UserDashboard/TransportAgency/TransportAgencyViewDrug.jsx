...
const TransportAgencyViewDrug = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://mediquick-server.onrender.com/api/v1/drug-basket/transportagency/drugs/${address}`
      );
  ...
  const handleDeleteDrug = (drug) => {
    fetch(
      `https://mediquick-server.onrender.com/api/v1/drug-basket/drug/${drug._id}`,
      {
        method: "DELETE",
      }
    ).then((data) => {
